...
def create_discrete_q_function(
    ...
    for _ in range(n_ensembles):
        ...
        # for p in cast(nn.Module, q_func._fc).parameters():
        #     p.register_hook(lambda grad: grad / n_ensembles)
...
def create_ensemble_normal_policy(
    observation_shape: Shape,
    action_size: int,
    encoder_factory: EncoderFactory,
    device: str,
    ...
    for _ in range(ensemble_num):
        ...
        pi_funcs.append(policy)
    pi_func_modules = nn.ModuleList(pi_funcs)
    pi_func_modules.to(device)
    pi_func_forwarder = EnsemblePiFunctionForwarder(pi_funcs)
    return pi_func_modules, pi_func_forwarder
