...
class QLearningAlgoBase(
    ...
    def predict_uncertainty(self, x: TorchObservation) -> torch.Tensor:
        # return self._imitator_forwarder.compute_uncertainty(x)
        torch_x = convert_to_torch_recursively(x, self._device)
        with torch.no_grad():
            if self._config.observation_scaler:
                torch_x = self._config.observation_scaler.transform(torch_x)

            action = self._impl.inner_predict_uncertainty(torch_x)
            if self._config.action_scaler:
                action = self._config.action_scaler.reverse_transform(action, just_scaling=True)
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if evaluators:
                for name, evaluator in evaluators.items():
                    ...
                    eval_hist.append(test_score)
                    # sort by eval-environment/rew
                    if len(eval_hist) >= compute_top_N and 'rew' in test_score:
                        eval_hist.sort(key=lambda x: x['rew'], reverse=True)
                        eval_hist = eval_hist[:compute_top_N]
                        # plot top_N averaged performance
                        top_N_mean = {k: np.mean([x[k] for x in eval_hist]) for k in eval_hist[0].keys()}
                        for k, v in top_N_mean.items():
                            logger.add_metric(f"eval-top_{compute_top_N}/{k}", v)
            # save metrics
            metrics = logger.commit(epoch, total_step)

            # save model parameters
            if epoch % save_interval == 0:
                if 'rew' in test_score:
                    logger.save_model("rew_" + str(test_score['rew']) + "&step_" + str(total_step), self)
                else:
