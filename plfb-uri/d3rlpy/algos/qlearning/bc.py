...
__all__ = ["BCConfig", "BC", "DiscreteBCConfig", "DiscreteBC", "EnsembleBCConfig", "EnsembleBC"]
...
# ensemble BC
@dataclasses.dataclass()
class EnsembleBCConfig(LearnableConfig):
    r"""Config of Behavior Cloning algorithm.
    ...
    def create(self, device: DeviceArg = False) -> "EnsembleBC":
        return EnsembleBC(self, device)
...
class EnsembleBC(QLearningAlgoBase[BCBaseImpl, EnsembleBCConfig]):
    def inner_create_impl(
        self, observation_shape: Shape, action_size: int
    ) -> None:
        # TODO: init with large std
        imitators, imitator_forwarder = create_ensemble_normal_policy(
                observation_shape,
                action_size,
                self._config.encoder_factory,
                min_logstd=np.log(0.01),
                max_logstd=np.log(0.5),
                device=self._device,
                ensemble_num=self._config.ensemble_num
            )

        optim = self._config.optim_factory.create(
            imitators.named_modules(), lr=self._config.learning_rate
        )

        modules = BCModules(optim=optim, imitator=imitators)

        self._impl = EnsembleBCImpl(
            observation_shape=observation_shape,
            action_size=action_size,
            modules=modules,
            device=self._device,
            action_min=self._config.action_min,
            action_max=self._config.action_max,
            imitator_forwarder=imitator_forwarder
        )
...
register_learnable(EnsembleBCConfig)
