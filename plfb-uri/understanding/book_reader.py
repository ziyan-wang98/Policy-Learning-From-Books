...
class BaseBooks2Knowledge(object):
    def __init__(self, book_path, book_res_path, cached_book_name, 
        ...
        self.agg_res_prefix = agg_res_prefix
    ...
    def book_level_knowledge_extraction(self, reuse_cache=True):
        ...
        for book_data in self.all_books_data:
            ...
            old_version_result_path = os.path.join(self.book_res_path, book_name, 'raw_results.npz')
            # mkdir 
            ...
            elif reuse_cache and os.path.exists(old_version_result_path):
                # be compatible with old version code.
                book_res = np.load(old_version_result_path, allow_pickle=True)['res']
    ...
    def code_aggregation(self, all_pre_agg_res, reuse_cache=True):
        ...
        while True:
            ...
            saved_agg_file = os.path.join(self.book_res_path, 'multi', f'{self.agg_res_prefix}-level-{level}.jsonl')
...
class FootballBooks2Knowledge(BaseBooks2Knowledge):
    def __init__(self, *args, **kwargs) -> None:
        ...
        # This is a precomputed results for the books with large-enough paragraphs that related to MDPs.
        # We use this to filter out books that are not related to MDPs to reduce the cost of querying.
        # You can set this to None to query all books.
        self.cached_related_book = ["Duncan Adams - Football Grounds a Fans' Guide [Retail 9781782814207]", 'The Soccer Goalkeeping Handbook (3rd Edition) - Alex Welsh', 'How to Watch Soccer - Ruud Gullit', 'Mike Smith - The Road to Glory', 'Football Intelligence']
        ...
        for paragraph in tqdm(paragraphs, desc="Processing Chapter"):
            if paragraph.startswith('#'):
                if current_chapter: # finish the previous chapter
                    ...
                    print("------") 
                    yield chapter_index, current_chapter
                    current_chapter = "" 
                    chapter_index += 1
    ...
    def paragraph_generator(self, book_data):
        ...
        for paragraph in data:
            if np.random.rand() < self.skip_rate:
                continue
            # TODO: This legacy key is policy_knowledge for compatibility, but the content mixes policy, dynamics, and reward knowledge and should be split later.
