...
class UncertaintyPredictor(object):
    ...
    def load_model(self, model_path=None) -> None:
        if model_path:
            ...
            if not os.path.isfile(model_path):
                raise FileNotFoundError(f"explicit uncertainty model checkpoint not found: {model_path}")
            print("load explicit path", model_path)
            self.learned_model = d3rlpy.load_learnable(model_path)
            return True
    ...
    def train_model(self, obs_list, action_list, next_obs_list, reward_list, end_epi_list, done_list,
                    batch_size=4096, n_steps=50_000, n_steps_per_epoch=1000):
        # ensemble model
        # obs + 1 (action)
        ...
        dtype=[np.float32],
        shape=[obs_list[0].shape[-1] + 1],  # type: ignore
        )
        # next_obs_len + 1 (reward)
        action_signature = Signature(
            dtype=[np.float32],
            shape=[next_obs_list[0].shape[-1] + 1],  # next_obs_list + 1
        )
        ...
        bc.fit(train_dataset, n_steps=int(n_steps), n_steps_per_epoch=n_steps_per_epoch, save_interval=10,
    ...
    def unc_rew_v2(self, env_rew, u_t, u_r):
        return ((1-self.coef_t-self.coef_r) * env_rew - (self.coef_t * u_t + self.coef_r * u_r)) / 2
