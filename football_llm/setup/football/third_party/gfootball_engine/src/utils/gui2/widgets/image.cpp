...
SDL_Surface *CreateMissingImageFallback(const std::string &name,
                                        size_t bytes_read) {
  std::cerr << "GFootball failed to load BMP image '" << name << "' ("
            << bytes_read << " bytes, SDL error: " << SDL_GetError() << ")"
            << std::endl;
  if (GetGameConfig().render) {
    throw std::runtime_error("GFootball failed to load BMP image: " + name);
  }

  SDL_Surface *fallback =
      SDL_CreateRGBSurfaceWithFormat(0, 1, 1, 32, SDL_PIXELFORMAT_ABGR8888);
  if (!fallback) {
    throw std::runtime_error(
        "GFootball failed to create fallback BMP surface: " + name);
  }
  return fallback;
}
...
#endif
  if (!image) {
    return CreateMissingImageFallback(name, bytes_read);
  }
