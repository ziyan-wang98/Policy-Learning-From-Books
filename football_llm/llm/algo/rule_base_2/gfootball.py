...
def find_patterns_dict(obs, player_x, player_y):
    """ find list of appropriate patterns in groups of memory patterns """
    for get_group in groups_of_memory_patterns:
        group = get_group(obs, player_x, player_y)
        if group["environment_fits"](obs, player_x, player_y):
            return {"patterns": group["get_memory_patterns"](obs, player_x, player_y), "group_name": get_group.__name__}
...
def get_action_dict_of_agent(obs, player_x, player_y):
    memory_patterns_dict = find_patterns_dict(obs, player_x, player_y)
    group_name = memory_patterns_dict["group_name"]
    memory_patterns = memory_patterns_dict["patterns"]
    # find appropriate pattern in list of memory patterns
    for get_pattern in memory_patterns:
        pattern = get_pattern(obs, player_x, player_y)
        if pattern["environment_fits"](obs, player_x, player_y):
            return {"action": pattern["get_action"](obs, player_x, player_y), "pattern": get_pattern.__name__, "group_name": group_name}
...
def agent_dict(obs, ret_dict=False, read_json=False):
    ...
    # get action of appropriate pattern in agent's memory
    if ret_dict:
        action_dict = get_action_dict_of_agent(obs, controlled_player_pos[0], controlled_player_pos[1])
        # return action
        return action_dict
    else:
        action = get_action_of_agent(obs, controlled_player_pos[0], controlled_player_pos[1])
...
def get_memory_patterns_new(obs, player_x, player_y):
    """ get action of appropriate pattern in agent's memory """
    memory_patterns = find_patterns(obs, player_x, player_y)
