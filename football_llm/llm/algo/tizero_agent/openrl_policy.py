...
class ObsEncoder(nn.Module):
    def __init__(self, input_embedding_size, hidden_size, _recurrent_N, _use_orthogonal, rnn_type):
        ...
        self.input_encoder = InputEncoder()     # Encode input first.
        self.input_embedding = get_fc(input_embedding_size, hidden_size)  # Embed encoder output.
        self.rnn = RNNLayer(hidden_size, hidden_size, _recurrent_N, _use_orthogonal, rnn_type=rnn_type)  # Pass embedded output through the RNN.
        self.after_rnn_mlp = get_fc(hidden_size, hidden_size)   # Apply the MLP after the RNN.
...
class PolicyNetwork(nn.Module):
    def __init__(self, device=torch.device("cpu")):
        ...
        self.predict_id = get_fc(self.hidden_size + self.action_dim, self.id_max)    # Predict ID from non-active-ID features after the RNN and MLP.
        self.id_embedding = get_fc(self.id_max, self.id_max)     # Use active ID as input and concatenate its output with other features.
