...
def my_controller(obs_list):
    idx = obs_list['active'] 
    action_list = agent.get_action(obs_list,idx)
    action = action_list[0].index(1)
    return action

def my_controller_multi(obs_list):
    res = []
    for i in range(10):
        action_list = agent.get_action(obs_list[i],i)
        action = action_list[0].index(1)
        res.append(action)
    return res
