...
from rehearsing.llm_mdp import RolloutMDP, TicTacToeRolloutMDP
...
def arg_parser():
    ...
    parser.add_argument('--rollout_num', type=int, default=1000)
    ...
    parser.add_argument('--oppo_strategy', type=str, default='minimax')
    # add bool argument
...
if __name__ == '__main__':
    ...
    state_field_saved_path = os.path.join(args.data_path, 'field_v2')
    ...
    if args.env_name == 'tictactoe':
        ...
        rollout_model = TicTacToeRolloutMDP(env=env, model=args.gen_model, seed=args.seed, 
                                            skip_knowledge=False, simulator_info=simulator_info, oppo_strategy=args.oppo_strategy)
        one_step_using = True
    elif args.env_name == 'football':
        one_step_using = False
    else:
        one_step_using = True
    cache_name = 'cache-v4'
    TTT_ROOT = os.environ.get('PLFB_TTT_DATA_PATH', os.path.join(os.environ.get('PLFB_ARTIFACT_ROOT', 'plfb_artifacts'), 'tic_tac_toe_data'))
    def code_int_creator(corpus, knowledge_type):
        code_instantiated_path = os.path.join(TTT_ROOT, 'instantiated_code', knowledge_type, cache_name)
        ...
        code_init = CodeInstantiation(retv, code_instantiated_path, env.vector_state_to_text, knowledge_type=knowledge_type,
                          ...
                          obs_desc=simulator_info.OBSERVATION_SPACE_DESC, acs_desc=simulator_info.ACTION_SPACE_DESC, one_step_using=one_step_using)
        return code_init
    ...
    imaginary_data_saved_path = os.path.join(TTT_ROOT, 'imaginary_data-v2')
    os.makedirs(imaginary_data_saved_path, exist_ok=True)
    img_data_save_file = os.path.join(imaginary_data_saved_path, f"seed-{args.seed}-oppo_strategy-{args.oppo_strategy}.npz")
    if os.path.exists(img_data_save_file):
        img_res_dict = np.load(os.path.join(imaginary_data_saved_path, f"seed-{args.seed}.npz"), allow_pickle=True)['data'].flatten()[0]
    print(f"current number of trajectories: {len(img_res_dict['trajectories'])}")
    for i in tqdm.tqdm(range(rollout_num-len(img_res_dict['trajectories']))):
        img_res_dict['trajectories'].append([])
        cur_vec_obs = copy.deepcopy(initial_state)
        player = 1
        other_information = {
            'oppo_player_symbol': 2,
        }
        for j in range(horizon):
            cur_vec_obs_with_player = np.concatenate([cur_vec_obs, [player]])
            text_obs = env.vector_state_to_text(cur_vec_obs_with_player)
            pi_prompt_code = Pi_code_init.get_code(cur_vec_obs_with_player)
            m_prompt_code = M_code_init.get_code(cur_vec_obs_with_player)
            r_prompt_code = R_code_init.get_code(cur_vec_obs_with_player)
            im_next_obs, im_action, im_llm_reward, thought_dict = rollout_model.rollout_one_step([cur_vec_obs_with_player], pi_prompt_code, r_prompt_code, m_prompt_code, other_information)
            # winner checker.
            done = False
            ending, winner = env.check_win(im_next_obs, ret_player=True)
            ending_rew = 0
            ending_str = 'None'
            if ending:
                done = True
                if winner == player:
                    ...
                    ending_rew = -1
                ending_str = 'winner is player' + str(winner)
            if cur_vec_obs[im_action] != 0:
                done = True
                ending_rew = -1
                ending_str = 'invalid action'
            if env.available_moves(cur_vec_obs) == 0:
                done = True
                ending_rew = 0
                ending_str = 'draw'
            img_res_dict['trajectories'][-1].append({'obs': cur_vec_obs, 'next_obs': im_next_obs, 'action': im_action, 
                                                 'dense_reward': im_llm_reward, 'reward': ending_rew, 
                                                 'done': done, 'thought': thought_dict, 'player': player, 'ending': ending_str})
            print(f"player {player} action {im_action} llm reward {im_llm_reward} ending {ending_str}")
            cur_vec_obs = im_next_obs
            if done:
                break
        # write to file
        np.savez(img_data_save_file, data=img_res_dict)