...
def compute_ensemble_stochastic_imitation_loss(
    policy: NormalPolicy, x: TorchObservation, action: torch.Tensor
) -> torch.Tensor:
    loss = torch.zeros(1, device=get_device(x))
    for p in policy:
        dist = build_gaussian_distribution(p(x))
        loss += F.mse_loss(dist.sample(), action)
        # TODO: compute weighting.
