...
from ...torch import NormalPolicy
...
class EnsemblePiFunctionForwarder:
    def __init__(self, pi_funcs: List[NormalPolicy]):
        self._pi_funcs = pi_funcs
    
    def sample_action(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            actions = []
            for pi_func in self._pi_funcs:
                action = pi_func.sample_action(x)
                actions.append(action.view(1, -1))
            return torch.mean(actions, dim=0)
    ...
    def best_action(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            actions = []
            for pi_func in self._pi_funcs:
                action = pi_func(x).squashed_mu
                actions.append(action.unsqueeze(0))
            return torch.mean(torch.cat(actions, dim=0), dim=0)
        
    def uncertainty(self, x: torch.Tensor, reduction: str='max') -> torch.Tensor:
        with torch.no_grad():
            actions = []
            for pi_func in self._pi_funcs:
                action = torch.exp(pi_func(x).logstd)
                actions.append(action.unsqueeze(0))
            return _reduce_ensemble(torch.cat(actions, dim=0), reduction, dim=0)
