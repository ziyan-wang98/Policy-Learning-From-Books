...
def rule_based_postprocess(wrapper_obs, obs, action, version='all_replaced'):
    ...
    from llm.utils.obs2text import get_zons_240
        ...
        for rtz in obs['right_team']:
            ...
            if version == 'all_replaced':
                ...
                if active_player_zone == zone:
                    # Trust additional opportunities or risks discovered by the policy
                    if action not in [9, 10, 11, 12, 13]:
                        action = rule_action
                        replace = True
                    # If the policy also detects an emergency, still replace the action
...
def get_rag_retriever(policy_path=None):
    ...
    if policy_path is None:
        policy_path = os.environ.get("PLFB_RAG_POLICY_PATH")
    if policy_path is None:
        dataset_path = os.environ.get("PLFB_DATASET_PATH")
        if dataset_path:
            policy_path = str(Path(dataset_path) / "rag")
        else:
            policy_path = str(Path(__file__).resolve().parents[3] / "data" / "rag")
    if not Path(policy_path).is_dir():
        raise FileNotFoundError(
            f"RAG policy path does not exist: {policy_path}. "
            "Set PLFB_RAG_POLICY_PATH or PLFB_DATASET_PATH."
        )
...
def evaluate_llm_with_environment(
    ...
    from gfootball.env.wrappers import Simple115StateWrapper_ball_owned_player
    from llm.algo.rule_base_2.gfootball import agent_dict as rule_base_2_agent_dict
    from llm.llm_baseline import RT2, llm_agent, llm_rag
    from llm.utils.obs2text import get_zons_240, img_obs_to_text, imaginary_data_observation_v2
