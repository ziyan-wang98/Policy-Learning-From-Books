...
__all__ = ["SACConfig", "SAC", "DiscreteSACConfig", "DiscreteSAC", "DiscreteCQLSACConfig", "DiscreteCQLSAC"]
...
@dataclasses.dataclass()
class DiscreteCQLSACConfig(LearnableConfig):
    r"""
    Config of Soft Actor-Critic algorithm with Conservative Q-Learning for discrete action-space.
    """
    ...
    # TODO: what is conservative_weight? 
    ...
    def create(self, device: DeviceArg = False) -> "DiscreteCQLSAC":
        return DiscreteCQLSAC(self, device)

    @staticmethod
    def get_type() -> str:
        return "discrete_cql_sac"

class DiscreteCQLSAC(QLearningAlgoBase[DiscreteCQLSACImpl, DiscreteCQLSACConfig]):
    def inner_create_impl(
        self, observation_shape: Shape, action_size: int
    ) -> None:
        q_funcs, q_func_forwarder = create_discrete_q_function(
        ...
        log_alpha = create_parameter((1, 1), math.log(self._config.initial_alpha), device=self._device)
...
register_learnable(DiscreteCQLSACConfig)
