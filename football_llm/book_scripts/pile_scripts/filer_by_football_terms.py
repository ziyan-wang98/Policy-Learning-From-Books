...
# list datasets dir
dataset_dir = os.environ.get('PLFB_PILE_TERMS_DIR', './runs/pile_football_terms')
output_dir = os.environ.get('PLFB_PILE_TERMS2_DIR', './runs/pile_football_terms2')
...
for p in files:
    ...
    if len(x)>0 :
        ...
        # filter
        filtered_dataset = dataset.filter(lambda example: len(regex_terms.findall(str(example['text'])))>=3, batch_size=10000, num_proc=100)
